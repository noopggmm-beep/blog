...
export default async function InnovationPage() {
  ...
  const { recent, older } = getPostsByTagGroupedByDate("科技创新", lang, 7);
      ...
      <div className="py-12 sm:py-16">
        ...
        {recent.length === 0 ? (
          <div className="text-center py-16 text-[var(--muted)]">
            ...
            <p className="text-lg">{lang === "zh" ? "暂无近期新闻" : "No recent news"}</p>
          </div>
        ) : (
              ...
              <section key={group.date}>
                <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 sticky top-14 z-10 py-2 bg-[var(--bg)]/80 backdrop-blur-sm">
                  ...
                  <span className="text-sm font-normal text-[var(--muted)]">
                    ({group.posts.length} {lang === "zh" ? "条" : "items"})
                  </span>
                </h2>
        ...
        {older.length > 0 && (
          <div className="text-center mt-12 pt-8 border-t border-[var(--border)]">
            <Link
              href="/tags/科技创新"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-xl glass glass-hover text-sm"
            >
              <span className="text-xl">📂</span>
              <span>{lang === "zh" ? "历史新闻" : "History"}</span>
