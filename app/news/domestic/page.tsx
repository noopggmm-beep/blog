...
export default async function DomesticNewsPage() {
  ...
  const posts = getPostsByTag("国内十大新闻", lang);
        ...
        <section className="hero-bg relative overflow-hidden rounded-xl mb-12">
          <div className="relative z-[1] py-16 px-6 text-center">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">
              <span className="gradient-text">{t(lang, "colDomNews")}</span>
            </h1>
            <p className="mt-3 text-[var(--muted)] max-w-lg mx-auto">
              {t(lang, "colDomNewsDesc")} · {t(lang, "colPostsCount", { count: posts.length })}
            </p>
          </div>
        </section>
        <PostList posts={posts} />
        <div className="text-center mt-8">
          <Link href="/tags/国内十大新闻" className="inline-flex items-center gap-1.5 text-sm text-[var(--muted)] hover:text-[var(--accent)] transition-colors">
