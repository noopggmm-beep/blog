...
export default async function InternationalNewsPage() {
  ...
  const { recent, older } = getPostsByTagGroupedByDate("国际十大新闻", lang, 7);
      ...
      <div className="py-12 sm:py-16">
        <section className="hero-bg relative overflow-hidden rounded-xl mb-10">
          <div className="relative z-[1] py-12 px-6 text-center">
            ...
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight"><span className="gradient-text">{t(lang, "colIntlNews")}</span></h1>
            <p className="mt-3 text-[var(--muted)]">{t(lang, "colIntlNewsDesc")}</p>
          </div>
        </section>
        {recent.length === 0 ? (
        ...
        {older.length > 0 && (
          <div className="text-center mt-12 pt-8 border-t border-[var(--border)]">
            <Link href="/tags/国际十大新闻" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl glass glass-hover text-sm">
